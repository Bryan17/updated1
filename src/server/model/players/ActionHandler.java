...
public class ActionHandler {
	...
	public void firstClickObject(int objectType, int obX, int obY) {
		...
		switch(objectType) {
		/*case 9706://PvP Lever
			c.getDH().sendDialogues(59, 0);
		break;*/
		case 1750://Ladder Up Varrock Home
			c.getPA().movePlayer(3183, 3446, 1);
		break;
		case 1749://Ladder down Varrock Home
			c.getPA().movePlayer(3183, 3446, 0);
		break;
		case 3415:
		...
		c.sendMessage("You'll only be able to see Donators here.");
		break;
		case 6452:
 ...
 case 410:
		...
		break;
		case 1817:
			c.getPA().startTeleport(3096, 3479, 0, "modern");
		break;
		case 1814:
			//ardy lever
		...
		case 9369:
			c.sendMessage("This is still under construction");
...
/*
		...
		break;
		case 2878:
			c.getPA().movePlayer(2508, 4718, 0);
			c.sendMessage("If a stall doesn't work move to the other side of it then click it.");
		break;
		case 5960:
	...
	public void secondClickObject(int objectType, int obX, int obY) {
		...
		switch(objectType) {
			...
			case 2560://silk
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 15000, 30, 25);
			break;
			case 2563://fur
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 30000, 60, 50);
			break;
			case 2564://spice
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 50000, 100, 75);
			break;
			case 2565://silver
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 70000, 170, 90);
			break;
			case 2562://gem
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 100000, 100000, 99);
			break;
			
			case 6163://Donator Stall [1]
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 50000, 100, 50);
			break;
			case 6165://Donator Stall [2]
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 100000, 170, 80);
			break;
			case 6166://Donator Stall [3]
				c.turnPlayerTo(obX, obY);
				c.getThieving().stealFromStall(995, 200000, 100000, 99);
			break;
	...
	c.fishitem = -1;
		...
		switch(npcType) {
			case 1599://Slayer Teleport
				c.getPA().movePlayer(3428, 3536, 0);
			break;
			case 959:
			if (c.playerLevel[3] < c.getLevelForXP(c.playerXP[3]) && c.playerLevel[3] > 0) {
				c.playerXP[3] = c.getPA().getXPForLevel(99)+5;
				c.playerLevel[3] = c.getPA().getLevelForXP(c.playerXP[3]);
				c.getPA().refreshSkill(3);
				c.sendMessage("The medic carefully heals you...");
				c.gfx0(2250);
				c.startAnimation(899);
			} else if (c.playerLevel[3] > c.getLevelForXP(c.playerXP[3])) {
					c.playerLevel[3] = c.getLevelForXP(c.playerXP[3]);
					c.getPA().refreshSkill(3);
					c.sendMessage("It seem's that you are, already healed!");
			} else {
					c.sendMessage("It seem's that you are, already healed!");
			}
			break;
