		...
		while(EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					...
					} else if (token.equals("Veng")) {
						p.castVeng = Integer.parseInt(token2);							
	...
	**/
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("Veng = ", 0, 7);
			characterfile.write(Integer.toString(p.castVeng), 0, Integer.toString(p.castVeng).length());
			characterfile.newLine();
