	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		switch(packetType) {		
			...
			**/
			...
			Client e2 = (Client)Server.playerHandler.players[c.playerIndex];
			if(c.isInPvP() && e2.isInBank() || e2.isInPvP() && c.isInBank()) {
				c.getCombat().resetPlayerAttack();
				c.stopMovement();
			}
			if(c.trade11 > 0) {
			...
			**/
			...
			Client e3 = (Client)Server.playerHandler.players[c.playerIndex];
			if(c.isInPvP() && e3.isInBank() && castingSpellId == c.MAGIC_SPELLS[i][0]) {
				c.getCombat().resetPlayerAttack();
				c.stopMovement();
				return;
				}
